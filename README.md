...
* `Alpha_Loss` / `Alpha_Grad_Norm` / `Alpha_Pre_Clip`: Detailed auto-tuning telemetry for the entropy coefficient mechanism.
...
> **Note on Ablations**: Two standalone ablation scripts (`federated_main_sac_ablation_a.py` and `federated_main_sac_ablation_b.py`) are included to investigate buffer-starvation effects on highly volatile attacks like `sign_flip` by modifying the SAC update frequency and target entropy respectively.
